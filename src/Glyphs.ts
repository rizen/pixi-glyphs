...
> extends PIXI.Container {
  ...
  private _tokensFlat: SegmentToken[] | null = null;  // Cached flat tokens
  private _cachedTagKeys: string[] | null = null;  // Cached Object.keys(tagStyles)
  private _styleCache: Record<string, TextStyleExtended> | undefined;  // Lazy style cache (undefined during construction)
  private _constructed = false;  // Flag to enable caching after construction
  ...
  public get tokensFlat(): SegmentToken[] {
    // Cache the flattened tokens to avoid repeated .flat(Infinity) calls
    if (this._tokensFlat === null) {
      this._tokensFlat = this._tokens.flat(Infinity) as SegmentToken[];
    }
    return this._tokensFlat;
  }
  ...
  public setStyleForTag(
    ...
    // Invalidate tag keys cache if this is a new tag (only check if cache exists)
  ...
  public removeStylesForTag(tag: string, skipUpdate?: boolean): boolean {
    if (tag in this.tagStyles) {
      ...
      this._cachedTagKeys = null;  // Invalidate tag keys cache
  ...
  constructor(
    ...
    // Set styles directly without triggering update - we'll update once at the end
    this._tagStyles = tagStyles;

    if (this.options.imgMap) {
      // Pass skipUpdate=true since we'll update at the end
      this.createSpriteTemplatesFromSourceMap(this.options.imgMap, true);
    }

    // This triggers the single update for the entire constructor
    this.text = text;

    // Enable caching for subsequent updates (setText, etc.)
   ...
   * @param imgMap
   */
  protected createSpriteTemplatesFromSourceMap(imgMap: ImageSourceMap, skipUpdate?: boolean) {
    this._spriteTemplates = {};
    ...
    Object.entries(imgMap).forEach(([key, spriteSource]) => {
      ...
      // Pass skipUpdate=true to batch updates - we'll update once at the end
      this.setStyleForTag(key, style, true);
    });

    // Trigger a single update after all sprite templates are added (unless skipUpdate is true)
    this._needsUpdate = true;
    this.updateIfShould(skipUpdate);
  }
  ...
  public update(skipDraw?: boolean): ParagraphToken {
    ...
    // Use cached tag keys only after construction (avoids overhead during init)
    let tagKeys: string[];
    if (this._constructed) {
      if (this._cachedTagKeys === null) {
        this._cachedTagKeys = Object.keys(this.tagStyles);
      }
      tagKeys = this._cachedTagKeys;
    } else {
      tagKeys = Object.keys(this.tagStyles);
    }
    ...
    // Assign styles to each segment. Pass style cache only after construction.
    const styledTokens = mapTagsToStyles(
      tagTokensNew,
      tagStyles,
      spriteTemplates,
      this._styleCache  // undefined during construction, populated after
    );
    ...
    this._tokensFlat = null;  // Invalidate cached flat tokens
    this._needsDraw = true;
